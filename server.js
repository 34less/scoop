...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body 
  	&& requestComment.articleId && requestComment.username) {
      const comment = {
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    let indexArticle = database.articles[savedComment.articleId].commentIds.indexOf(id);
    database.articles[savedComment.articleId].commentIds.splice(indexArticle,1);
    let indexUser = database.users[savedComment.username].commentIds.indexOf(id);
    database.users[savedComment.username].commentIds.splice(indexUser,1);
